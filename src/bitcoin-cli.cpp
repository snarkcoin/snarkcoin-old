...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to snarkcoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:6327/wallet/<walletname>", false, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  snarkcoin-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     snarkcoin-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     snarkcoin-cli [options] help                List commands\n"
                "or:     snarkcoin-cli [options] help <command>      Get help for a command\n";
