...
Snarkcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play snarkcoins" on the test network, if you
...
([`valgrind.supp`](https://github.com/snarkcoin-project/snarkcoin/blob/master/contrib/valgrind.supp))
...
To profile a running snarkcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep snarkcoind` -- sleep 60
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 6328.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6327 for connections and services them.
...
Some of these are maintained by active developers of Snarkcoin Core, in which case changes should probably go
...
file descriptors it will cause problems with Snarkcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Snarkcoin's
